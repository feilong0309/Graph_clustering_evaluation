...
def handleArgs():
    ...
    parser.add_argument("-sample", "--samplerate", default=1.0, type=float, help="the sample percentage of the entire graph", dest="sample_percentage")
...
def clusterByLancichinetti(n, p, f, c, program_path):
    ...
    mylist=['time', 'python', 'select.py', '-n', n, '-p', str(p), '-f', f, '-c', str(c)]
...
def clusterByLeiden(input_file, output_file, random_seed, modularity_function = 1, resolution_parameter = 1.0, optimization_algorithm = 3, n_random_starts = 10, n_iterations = 10, print_output = 0):
    ...
    mylist=['time', 'java', args.xmx, '-jar', 'ModularityOptimizer.jar', input_file, output_file, str(modularity_function), str(resolution_parameter), str(optimization_algorithm), str(n_random_starts), str(n_iterations), str(random_seed), str(print_output)]
...
#    import sys
#    sys.path.append(args.demon_path)
#    import Demon
#    import launch

    retcode=subprocess.call(['python', 'launch.py', input_file])
...
#    parseDemonResults(args.demon_path + '/communities', out_file_prefix, out_file_number, out_file_suffix, out_file_separator, out_path)
    parseDemonResults('communities', out_file_prefix, out_file_number, out_file_suffix, out_file_separator, out_path)
        ...
        for i in xrange(len(graph_files)):
            ...
            for t in xrange(args.numtrials):
                ...
                if method == "blondel" or method == "louvain":
                    runLancichinettiClustering(os.getcwd() + '/' + graph_files[i], 4, os.getcwd() + '/' + scratch_directory_stem, 1, args.lancichinetti_program_path, method + '_p' + str(int(100*sample_percentage)) + '_' + out_file_prefix, str(i + args.file_range_start) + '_' +  str(t + 1), out_file_suffix, out_file_separator, args.out_directory_stem)

                elif method == "label_propagation":
                    runLancichinettiClustering(os.getcwd() + '/' + graph_files[i], 5, os.getcwd() + '/' + scratch_directory_stem, 1, args.lancichinetti_program_path, method + '_p' + str(int(100*sample_percentage)) + '_' + out_file_prefix, str(i + args.file_range_start) + '_' +  str(t + 1), out_file_suffix, out_file_separator, args.out_directory_stem)

                elif method == "modularity_optimization":
                    runLancichinettiClustering(os.getcwd() + '/' + graph_files[i], 8, os.getcwd() + '/' + scratch_directory_stem, 1, args.lancichinetti_program_path, method + '_p' + str(int(100*sample_percentage)) + '_' + out_file_prefix, str(i + args.file_range_start) + '_' +  str(t + 1), out_file_suffix, out_file_separator, args.out_directory_stem)

                elif method == "slm":
                    runLeidenClustering(os.getcwd() + '/' + graph_files[i], args.lancichinetti_program_path + 'modopt_output.txt', random.randrange(0, 1000000000000000000), method + '_p' + str(int(100*sample_percentage)) + '_' + out_file_prefix, str(i + args.file_range_start)+ '_' +  str(t + 1), out_file_suffix, out_file_separator, args.out_directory_stem)
                    
                elif is_directed:
                    if method == "oslom":
                        runLancichinettiClustering(os.getcwd() + '/' + graph_files[i], 1, os.getcwd() + '/' + scratch_directory_stem, 1, args.lancichinetti_program_path, method + '_p' + str(int(100*sample_percentage)) + '_' + out_file_prefix, str(i + args.file_range_start) + '_' +  str(t + 1), out_file_suffix, out_file_separator, args.out_directory_stem)

                    elif method == "infomap":
                        runLancichinettiClustering(os.getcwd() + '/' + graph_files[i], 3, os.getcwd() + '/' + scratch_directory_stem, 1, args.lancichinetti_program_path, method + '_p' + str(int(100*sample_percentage)) + '_' + out_file_prefix, str(i + args.file_range_start) + '_' +   str(t + 1), out_file_suffix, out_file_separator, args.out_directory_stem)

                    elif method == "hierarchical_infomap":
                        raise
                        #Implement correctly for hierarchical
                        runLancichinettiClustering(os.getcwd() + '/' + graph_files[i], 7, os.getcwd() + '/' + scratch_directory_stem, 1, args.lancichinetti_program_path, method + '_p' + str(int(100*sample_percentage)) + '_' + out_file_prefix, str(i + args.file_range_start) + '_' +  str(t + 1), out_file_suffix, out_file_separator, args.out_directory_stem)
                        
                else:
                    if method == "oslom":
                        runLancichinettiClustering(os.getcwd() + '/' + graph_files[i], 0, os.getcwd() + '/' + scratch_directory_stem, 1, args.lancichinetti_program_path, method + '_p' + str(int(100*sample_percentage)) + '_' + out_file_prefix, str(i + args.file_range_start) + '_' +  str(t + 1), out_file_suffix, out_file_separator, args.out_directory_stem)

                    elif method == "infomap":
                        runLancichinettiClustering(os.getcwd() + '/' + graph_files[i], 2, os.getcwd() + '/' + scratch_directory_stem, 1, args.lancichinetti_program_path, method + '_p' + str(int(100*sample_percentage)) + '_' + out_file_prefix, str(i + args.file_range_start) + '_' +  str(t + 1), out_file_suffix, out_file_separator, args.out_directory_stem)

                    elif method == "hierarchical_infomap":
                        #raise
                        #Implement correctly for hierarchical
                        runLancichinettiClustering(os.getcwd() + '/' + graph_files[i], 6, os.getcwd() + '/' + scratch_directory_stem, 1, args.lancichinetti_program_path, method + '_p' + str(int(100*sample_percentage)) + '_' + out_file_prefix, str(i + args.file_range_start) + '_' +  str(t + 1), out_file_suffix, out_file_separator, args.out_directory_stem)

                    elif method == "demon":
                        runDemonClustering(os.getcwd() + '/' + graph_files[i], method + '_p' + str(int(100*sample_percentage)) + '_' + out_file_prefix, str(i + args.file_range_start) + '_' +  str(t + 1), out_file_suffix, out_file_separator, args.out_directory_stem)
